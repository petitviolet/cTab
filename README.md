...
- **ウィンドウを閉じる / アプリを終了** — スイッチャー表示中、選択中のウィンドウに対して `Command+W`（閉じる）/ `Command+Q`（アプリ終了）
...
- **アクティブディスプレイの識別** — 複数ディスプレイ環境で、各ウィンドウがマウスカーソルのある画面にあるかどうかを判別。アクティブ画面にあるウィンドウは通常表示、他の画面にあるウィンドウは減光して見分けやすくする（マウス移動にリアルタイム追従）。設定でON/OFFと減光の濃さを調整可能
- **設定ウィンドウ** — アプリ起動時に表示。権限状態の確認、ログイン起動の切替、表示サイズ調整、全ディスプレイ表示の切替、アクティブディスプレイ強調の切替・濃さ調整、使い方、**再起動**・終了ができる
